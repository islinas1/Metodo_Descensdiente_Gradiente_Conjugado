...
def ackley(x):
    ...
    b = -np.exp(0.5*(np.cos(2*np.pi*x[0]) + np.cos(2*np.pi*x[1])))
    return a + b + np.e + 20
...
def grad_ackley(x):
    ...
    exp_r = np.exp(-0.2*r)
    exp_c = np.exp(0.5*(np.cos(2*np.pi*x[0]) + np.cos(2*np.pi*x[1])))
    dr_dx = 0.5*x[0]/r
    dr_dy = 0.5*x[1]/r
    dx = 20*0.2*exp_r*dr_dx + np.pi*np.sin(2*np.pi*x[0])*exp_c
    dy = 20*0.2*exp_r*dr_dy + np.pi*np.sin(2*np.pi*x[1])*exp_c
    return np.array([dx, dy])

def levy(x):
    w0 = 1 + (x[0]-1)/4
    w1 = 1 + (x[1]-1)/4
    t1 = np.sin(np.pi*w0)**2
    t2 = (w0-1)**2 * (1 + 10*np.sin(np.pi*w0+1)**2)
    t3 = (w1-1)**2 * (1 + np.sin(2*np.pi*w1)**2)
    return t1 + t2 + t3

def grad_levy(x):
    h = 1e-7
    dx = (levy(x + np.array([h,0])) - levy(x - np.array([h,0]))) / (2*h)
    dy = (levy(x + np.array([0,h])) - levy(x - np.array([0,h]))) / (2*h)
    return np.array([dx, dy])
...
def grad_goldstein_price(x):
    h = 1e-7
    dx = (goldstein_price(x + np.array([h,0])) - goldstein_price(x - np.array([h,0]))) / (2*h)
    dy = (goldstein_price(x + np.array([0,h])) - goldstein_price(x - np.array([0,h]))) / (2*h)
    return np.array([dx, dy])

def cuadratica_coseno(x):
    return x[0]**2 + x[1]**2 + 2*np.cos(x[0])*np.cos(x[1])

def grad_cuadratica_coseno(x):
    dx = 2*x[0] - 2*np.sin(x[0])*np.cos(x[1])
    dy = 2*x[1] - 2*np.cos(x[0])*np.sin(x[1])
    return np.array([dx, dy])

CATALOGO_FUNCIONES = {
    "Esfera": {
        "f": esfera, "grad": grad_esfera,
        "latex": "f(x,y) = x² + y²",
        "minimo": np.array([0.0, 0.0]), "f_min": 0.0,
        "x0": np.array([-3.0, -3.0]), "rango": 5,
        "info": "La mas simple. Cuenco circular perfecto. Referencia base.",
        "dificultad": "Facil"
    },
    "Booth": {
        "f": booth, "grad": grad_booth,
        "latex": "f(x,y) = (x + 2y - 7)² + (2x + y - 5)²",
        "minimo": np.array([1.0, 3.0]), "f_min": 0.0,
        "x0": np.array([-3.0, -3.0]), "rango": 6,
        "info": "Cuadratica rotada. Parece simple pero los ejes estan inclinados.",
        "dificultad": "Facil"
    },
    "Matyas": {
        "f": matyas, "grad": grad_matyas,
        "latex": "f(x,y) = 0.26(x² + y²) - 0.48xy",
        "minimo": np.array([0.0, 0.0]), "f_min": 0.0,
        "x0": np.array([5.0, -5.0]), "rango": 8,
        "info": "Valle casi plano. Prueba la precision del metodo.",
        "dificultad": "Facil"
    },
    "Cuadratica + Coseno": {
        "f": cuadratica_coseno, "grad": grad_cuadratica_coseno,
        "latex": "f(x,y) = x² + y² + 2·cos(x)·cos(y)",
        "minimo": np.array([0.0, 0.0]), "f_min": 2.0,
        "x0": np.array([-3.0, 2.0]), "rango": 5,
        "info": "Cuenco con ondulaciones. Los cosenos crean minimos locales.",
        "dificultad": "Media"
    },
    "Three-Hump Camel": {
        "f": three_hump_camel, "grad": grad_three_hump_camel,
        "latex": "f(x,y) = 2x² - 1.05x⁴ + x⁶/6 + xy + y²",
        "minimo": np.array([0.0, 0.0]), "f_min": 0.0,
        "x0": np.array([2.0, -1.5]), "rango": 4,
        "info": "Tiene 3 jorobas. La funcion sube, baja y vuelve a subir.",
        "dificultad": "Media"
    },
    "Rosenbrock": {
        "f": rosenbrock, "grad": grad_rosenbrock,
        "latex": "f(x,y) = (1 - x)² + 100(y - x²)²",
        "minimo": np.array([1.0, 1.0]), "f_min": 0.0,
        "x0": np.array([-1.5, -1.0]), "rango": 4,
        "info": "Valle curvo largo y estrecho. El benchmark mas famoso de optimizacion.",
        "dificultad": "Dificil"
    },
    "Himmelblau": {
        "f": himmelblau, "grad": grad_himmelblau,
        "latex": "f(x,y) = (x² + y - 11)² + (x + y² - 7)²",
        "minimo": np.array([3.0, 2.0]), "f_min": 0.0,
        "x0": np.array([-3.0, -3.0]), "rango": 6,
        "info": "Tiene 4 minimos: (3,2), (-2.8,3.1), (-3.8,-3.3), (3.6,-1.8). El resultado depende del punto inicial.",
        "dificultad": "Dificil"
    },
    "Beale": {
        "f": beale, "grad": grad_beale,
        "latex": "f(x,y) = (1.5-x+xy)² + (2.25-x+xy²)² + (2.625-x+xy³)²",
        "minimo": np.array([3.0, 0.5]), "f_min": 0.0,
        "x0": np.array([-1.0, -1.0]), "rango": 5,
        "info": "Valle plano alargado. El gradiente casi desaparece cerca del minimo.",
        "dificultad": "Dificil"
    },
    "Rastrigin": {
        "f": rastrigin, "grad": grad_rastrigin,
        "latex": "f(x,y) = 20 + x² + y² - 10(cos(2πx) + cos(2πy))",
        "minimo": np.array([0.0, 0.0]), "f_min": 0.0,
        "x0": np.array([2.5, 2.5]), "rango": 5,
        "info": "Llena de minimos locales falsos por los cosenos. Muy dificil para metodos de gradiente.",
        "dificultad": "Muy dificil"
    },
    "Ackley": {
        "f": ackley, "grad": grad_ackley,
        "latex": "f(x,y) = -20·exp(-0.2·sqrt(0.5(x²+y²))) - exp(0.5(cos(2πx)+cos(2πy))) + e + 20",
        "minimo": np.array([0.0, 0.0]), "f_min": 0.0,
        "x0": np.array([3.0, 3.0]), "rango": 5,
        "info": "Superficie con muchos hoyos enganosos. Solo el centro es el minimo real.",
        "dificultad": "Muy dificil"
    },
    "Levy": {
        "f": levy, "grad": grad_levy,
        "latex": "f(x,y) = sin²(πw₁) + (w₁-1)²(1+10sin²(πw₁+1)) + (w₂-1)²(1+sin²(2πw₂))",
        "minimo": np.array([1.0, 1.0]), "f_min": 0.0,
        "x0": np.array([-4.0, -4.0]), "rango": 6,
        "info": "Ondulaciones por todas partes. wᵢ = 1 + (xᵢ-1)/4.",
        "dificultad": "Muy dificil"
    },
    "Goldstein-Price": {
        "f": goldstein_price, "grad": grad_goldstein_price,
        "latex": "f(x,y) = [1+(x+y+1)²(19-14x+3x²-14y+6xy+3y²)]·[30+(2x-3y)²(...)]",
        "minimo": np.array([0.0, -1.0]), "f_min": 3.0,
        "x0": np.array([-1.5, 0.5]), "rango": 2.5,
        "info": "Valores enormes en casi todo el dominio. Minimo global en (0,-1) con f=3.",
        "dificultad": "Muy dificil"
    },
}
...
if seccion == "Inicio":
    ...
    **múltiples fórmulas** para el parámetro **βₖ**, cada una con distintas propiedades.
    ...
    nombres_funciones = list(CATALOGO_FUNCIONES.keys()) + ["Personalizada"]
    dificultades = {n: CATALOGO_FUNCIONES[n]["dificultad"] for n in CATALOGO_FUNCIONES}
    dificultades["Personalizada"] = "Tu funcion"
    opciones_display = [f"{n}  [{dificultades[n]}]" for n in nombres_funciones]

    col_cfg1, col_cfg2 = st.columns([2, 1])

    with col_cfg1:
        seleccion = st.selectbox("Funcion de prueba:", opciones_display)
        nombre_sel = nombres_funciones[opciones_display.index(seleccion)]

    es_personalizada = nombre_sel == "Personalizada"

    if es_personalizada:
        st.markdown("""
        <div class="highlight-box">
            <strong>Escribe tu funcion usando x e y.</strong> Funciones disponibles:
            sin, cos, tan, exp, log, sqrt, abs, pi. Operadores: + - * / ** ( )
        </div>
        """, unsafe_allow_html=True)

        col_f1, col_f2 = st.columns([3, 1])
        with col_f1:
            expr_usuario = st.text_input(
                "f(x, y) =",
                value="x**2 + y**2 + 2*cos(x)*cos(y)",
                key="custom_func"
            )
        with col_f2:
            rango_custom = st.number_input("Rango del grafico", value=5.0, step=1.0, min_value=1.0, key="custom_rango")

        with col_cfg2:
            x0_nl1 = st.number_input("x0[1]", value=-3.0, step=0.5, key="nl_x01")
            x0_nl2 = st.number_input("x0[2]", value=-3.0, step=0.5, key="nl_x02")
        ...
        try:
            ...
            eval(expr_usuario, {"__builtins__": {}, "x": x_test, "y": y_test, **funciones_seguras})
        except Exception as ex:
            st.error(f"Error en la funcion: {ex}")
            func_valida = False

        if func_valida:
            def f_custom(v):
                x, y = v[0], v[1]
                return float(eval(expr_usuario, {"__builtins__": {}, "x": x, "y": y, **funciones_seguras}))

            def grad_custom(v):
                h = 1e-7
                fx = f_custom(v)
                dx = (f_custom(v + np.array([h, 0])) - f_custom(v - np.array([h, 0]))) / (2*h)
                dy = (f_custom(v + np.array([0, h])) - f_custom(v - np.array([0, h]))) / (2*h)
                return np.array([dx, dy])

            f_test = f_custom
            grad_test = grad_custom
            nombre_f = "Personalizada"
            sol_exacta = np.array([0.0, 0.0])

            st.markdown(f"""
            <div class="formula-box">f(x, y) = {expr_usuario}</div>
            """, unsafe_allow_html=True)

            func_data = {
                "latex": f"f(x, y) = {expr_usuario}",
                "minimo": sol_exacta, "f_min": "?",
                "dificultad": "Personalizada", "rango": rango_custom,
                "info": "Funcion ingresada manualmente. El gradiente se calcula por diferencias finitas."
            }
            ...
            with col_info1:
                st.markdown(f"""
                <div class="step-card">
                    <span class="step-num">Punto inicial</span><br>
                    ({x0_nl1:.1f}, {x0_nl2:.1f})
                </div>
                """, unsafe_allow_html=True)
            with col_info2:
                try:
                    f_x0 = f_custom(np.array([x0_nl1, x0_nl2]))
                    st.markdown(f"""
                    <div class="step-card">
                        <span class="step-num">f(x0)</span><br>
                        {f_x0:.6f}
                    </div>
                    """, unsafe_allow_html=True)
                except Exception:
            ...
            st.markdown("""
            <div class="highlight-box">
                <strong>Personalizada:</strong> El gradiente se calcula automaticamente
                por diferencias finitas. El minimo no se conoce de antemano — el metodo
                lo buscara desde el punto inicial.
            </div>
            """, unsafe_allow_html=True)
        else:
            f_test = None
        ...
        func_data = CATALOGO_FUNCIONES[nombre_sel]

        with col_cfg2:
            x0_nl1 = st.number_input("x0[1]", value=float(func_data["x0"][0]), step=0.5, key="nl_x01")
            x0_nl2 = st.number_input("x0[2]", value=float(func_data["x0"][1]), step=0.5, key="nl_x02")

        f_test = func_data["f"]
        grad_test = func_data["grad"]
        nombre_f = nombre_sel
        sol_exacta = func_data["minimo"]

        st.markdown(f"""
        <div class="formula-box">{func_data["latex"]}</div>
        """, unsafe_allow_html=True)
        ...
        with col_info1:
            st.markdown(f"""
            <div class="step-card">
                <span class="step-num">Minimo</span><br>
                ({sol_exacta[0]:.2f}, {sol_exacta[1]:.2f}) con f = {func_data["f_min"]}
            </div>
            """, unsafe_allow_html=True)
        with col_info2:
            st.markdown(f"""
            <div class="step-card">
                <span class="step-num">Dificultad</span><br>
                {func_data["dificultad"]}
            </div>
            """, unsafe_allow_html=True)
        with col_info3:
            st.markdown(f"""
        ...
        st.markdown(f"""
        <div class="highlight-box">
            <strong>{nombre_f}:</strong> {func_data["info"]}
        </div>
        """, unsafe_allow_html=True)

    x0_nl = np.array([x0_nl1, x0_nl2])

    if not es_personalizada or (es_personalizada and f_test is not None):
        ...
        if len(variantes_sel) == 0:
            ...
            margin = max(abs(all_pts).max(), abs(x0_nl).max(), abs(sol_exacta).max(), func_data["rango"]) + 1
            margin = min(margin, 12)
